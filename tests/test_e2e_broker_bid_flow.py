"""
End-to-end bid flow: broker places a bid on an existing load
...
@allure.story("Broker places a bid and sees it in My Bids")
@allure.severity(allure.severity_level.CRITICAL)
def test_broker_can_place_bid_and_see_in_my_bids(logged_in_broker: Page):
    """
    Broker finds an available load, places a bid, then navigates
    to /my-bids and confirms the bid is listed.
    """
    page = logged_in_broker
    page.set_default_timeout(60000)
    BID_NOTE = "E2E broker bid"
    ...
    with allure.step("Open Place a bid form and submit"):
        ...
        if limit_modal.is_visible(timeout=3000):
            ...
            pytest.skip("Broker daily bid limit reached — reset the account or run tomorrow")
