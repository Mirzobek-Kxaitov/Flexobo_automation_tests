"""
E2E Multi-user Bid Reject Lifecycle (Phase 6) — bid rad etish.

Stsenariy:
1. Load_owner unique-price'li yuk yaratadi
2. Carrier bid yuboradi
3. Load_owner Received bids → o'sha bid'ni Reject qiladi
4. Verifikatsiya: carrier /my-bids/Rejected filterda bid'ini ko'radi

Texnika: Accept testiga o'xshash, lekin "Reject" tugmasi va "Rejected" filter.
Reject confirmation modal/reason input talab qilishi mumkin — agar fail bo'lsa,
diagnostika orqali aniqlaymiz.
"""
import os
import re
import allure
...
@allure.story("Phase 6: load_owner rejects carrier's bid → carrier sees Rejected")
...
def test_load_owner_rejects_bid_and_carrier_sees_rejected(
    ...
    - load_owner bid'ni Reject qiladi
    - carrier /my-bids/Rejected da bid'ni ko'radi
    """
    UNIQUE_PRICE = "19264"  # accept (27381) va flow (23579) dan farqli
    BID_NOTE = "E2E Lifecycle Reject bid"
    ...
    # ─── 3. Load_owner: Received bids → bid'ni Reject qilish ───────────
    ...
    # Reject tugmasini bosish
    logged_in_load_owner.get_by_role("button", name="Reject").click()
    logged_in_load_owner.wait_for_timeout(5000)

    # ─── 4. Carrier: /my-bids/Rejected da bid'ni ko'rish ───────────────
    logged_in_carrier.goto(f"{APP_URL}/my-bids")
    logged_in_carrier.wait_for_load_state("domcontentloaded")
    logged_in_carrier.wait_for_timeout(3000)

    # "Rejected" filter tab
    logged_in_carrier.get_by_text("Rejected", exact=True).first.click()
    logged_in_carrier.wait_for_timeout(3000)

    # Rejected ro'yxatda bizning yuk (UNIQUE_PRICE) ko'rinishi kerak
    expect(
