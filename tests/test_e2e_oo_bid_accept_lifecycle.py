"""
E2E OO bid accept lifecycle: load_owner creates a load, owner operator places a bid,
load_owner accepts the bid, owner operator sees it as Accepted in My bids.
"""
import os
import re
import pytest
...
@allure.story("Load owner accepts OO bid — OO sees Accepted")
@allure.severity(allure.severity_level.CRITICAL)
def test_load_owner_accepts_oo_bid(
    ...
    4. Owner Operator sees the bid as Accepted in My bids
    """
    ...
    with allure.step("Load owner clicks Accept on the OO's bid"):
        bid_card = (
    ...
    # Step 4: Owner Operator verifies bid is Accepted
    with allure.step("Owner Operator navigates to My bids and filters by Accepted"):
        oo.goto(f"{APP_URL}/my-bids")
        oo.wait_for_load_state("domcontentloaded")
        oo.wait_for_timeout(3000)

        oo.get_by_text("Accepted", exact=True).first.click()
        oo.wait_for_timeout(3000)
